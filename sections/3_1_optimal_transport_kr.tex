...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % ----------------------------------------------------
        % LEFT PANEL: Target Measure (Exact Parametric Shear)
        % ----------------------------------------------------
        \begin{scope}[shift={(0,0)}]
            ...
            % Exact Mathematical Contours: x_1 = c*cos(t), x_2 = c*sin(t) + 0.4*c^2*cos^2(t) - 0.5
            % Level 1 (Outer, c = 2.4)
            \draw[contour1] plot[domain=0:360, samples=100, smooth cycle] 
                ({2.4*cos(\x)}, {2.4*sin(\x) + 2.304*cos(\x)*cos(\x) - 0.5});
                
            % Level 2 (c = 1.8)
            \draw[contour2] plot[domain=0:360, samples=100, smooth cycle] 
                ({1.8*cos(\x)}, {1.8*sin(\x) + 1.296*cos(\x)*cos(\x) - 0.5});
                
            % Level 3 (c = 1.1)
            \draw[contour3] plot[domain=0:360, samples=100, smooth cycle] 
                ({1.1*cos(\x)}, {1.1*sin(\x) + 0.484*cos(\x)*cos(\x) - 0.5});
                
            % Level 4 (Inner Core, c = 0.5)
            \draw[contour4] plot[domain=0:360, samples=100, smooth cycle] 
                ({0.5*cos(\x)}, {0.5*sin(\x) + 0.100*cos(\x)*cos(\x) - 0.5});
            
            % Exact Transformed Empirical Ensemble Scatter
        ...
        \begin{scope}[shift={(8.0, 0.0)}]
            ...
            \node[mapped_particle] at (-2.1, -1.1) {};
            ...
            % Sub-caption
            \node[below=0.8cm of {(0,-3.0)}] {Reference measure, $\eta(z)$};
        \end{scope}

    \end{tikzpicture}
    \caption{A schematic illustrating optimal transport via a deterministic map $S$ pushing a sheared target measure forward to a standard normal reference.}
