...
Mathematically, we introduce a pruning threshold $\epsilon_p > 0$ and a pruning interval $K_{\mathrm{IHT}} \in \mathbb{N}$, ensuring the filtering operation only occurs every $K_{\mathrm{IHT}}$ iterations to allow the weights time to settle. We define a boolean active mask vector $\mathcal{M}^{(t)} \in \{0, 1\}^d$, initialised as a vector of ones, $\mathcal{M}^{(0)} = \mathbf{1}$. At iteration $t$, if $t \pmod {K_{\mathrm{IHT}}} \equiv 0$, the mask is updated via element-wise multiplication (Hadamard product) with an indicator function:
\begin{equation*}
...
\For{$t = 0, 1, \dots, T-1$}
    ...
    \If{$t > 0$ \textbf{and} $t \pmod {K_{\mathrm{IHT}}} \equiv 0$}
